...
Devがアプリケーションのデプロイをはじめとして、Opsの領域をカバーすることが
増えていく中で、Opsの領域が有していたノウハウが引き継なかったことによる、
ノウハウの断絶が今日の現場では生じています。

例えば作業時のダブルチェックや、バックアップ処理などの運用の定常処理に対する監視
などです。作業時のダブルチェックは、とかくソフトウェアエンジニアリングの省略化を
是とする価値観からは批判されがちですが、手順書に従って作業を行う際に、作業を行う側と、作業が
手順通りに行われていることを確認するという手順を分担するという考えは、れっきとした
合理性を有しています。

また開発プロセスの中で、運用に関する要件が未確定な状態で、ソフトウェア開発者が
運用の領域の課題をソフトウェアの領域でカバーしようとすることによる、
オーバーエンジニアリングの問題が生まれてきています。その結果として、問題なく動作する
ことを事前に確認することが難しい運用要件がいたずらに自動化されることにより、
...
一つのチーム内で多様な専門性を確保するためには、チームを構成するメンバーのバックグラウンドを
尊重できる組織作りと、時代の推移に応じて、メンバーである技術者のスキルを再生産するための
ゆとりが求められます。その意味においてDevOpsとは「DevとOpsが協力すること」なのであり、
自動化やクラウドなどのテクノロジーの活用を通じて、そのような組織作りを支えていくことが求められます。