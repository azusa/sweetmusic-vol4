...
「Devのスピード感のために、邪魔なOpsを排斥する」という思想ではありません。

また、「動くコードに手を入れるな」という言葉に象徴されるように、
Devがコードの構成に手を入れた場合にその影響範囲を読み切れないことから、
コードベースに積極的に手を入れることに慎重な態度を見せることもまたよくあることです。
その際、Opsは運用の中から生まれる課題を解決するためにシステムの改修を望んでいるわけです。

開発プロセスの中で、運用に関する要件が未確定な状態で、ソフトウェア開発者が
...
- 開発工程がボトルネック
- 開発環境(IDE/デバッガ)によりプログラミングの生産性が改善されると、テストやデプロイがネックになってくる
- 継続的デリバリー(CI)によりテストやデプロイの生産性が改善されると、インフラの構築がネックになってくる
- インフラのボトルネックを解消するにはクラウドや、Infrastrcture as Codeが有効

- システム開発の各工程を担う主体の中で、成果物を担うためのリードタイムにずれがあると、
- 個別工程の改善が最終的にデリバリーするまでの時間の短縮に結びつかず、個別最適になり、また各担当者の間の心理的軋轢を生み出す。
- DevOpsとはフィードバックサイクルを短縮し、価値あるプロダクトを作り出すために、開発とデリバリーのプロセスを全体最適化すること
...
ユーザー企業がプロジェクトをハンドルする中で、専門性を発揮してDevとOpsの間をインテグレーションするのが、
DevOps時代のシステムインテグレーターの役割
